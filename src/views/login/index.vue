...
import { mapActions } from 'vuex' // 引入 vuex的辅助函数
export default {
  name: 'Login',
  data() {
    ...
    //     callback(new Error('密码不能少于6位,且不能大于16位'))
    //   } else {
    //     callback()
    //   }
    // }
  ...
  methods: {
    ...mapActions(['user/login']), // 引入 mapActions=> 登录的login 的actions方法
    showPwd() {
      ...
      this.$refs.loginForm.validate(async isOK => {
        if (isOK) {
          try {
            ...
            console.log(err)
          } finally { // 不管执行 try 还是 catch,最后都会进入finally
            this.loading = false
          }
        }
      })
